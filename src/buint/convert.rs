...
/// Provides compile time check that the first given number is less than the
/// second given number.
const fn ensure_a_less_than_b(a: u32, b: u32) {
    assert!(a < b);
}

/// Implements `From` and `TryFrom` traits between two `BUint` types.
///
/// The `from` type must have fewer digits than the `to` types. For example,
/// `U256 => U512` is valid but `U512 => U256` isn't. This is checked at
/// compile time.
macro_rules! big_conversion {
    ($from:tt => $to:tt) => {
        // Make sure at compile time that `$from` is less than `$to`.
        const _: () = ensure_a_less_than_b($from::BYTES, $to::BYTES);

        impl From<$from> for $to {
            fn from(value: $from) -> Self {
                // Note: In bnum, the digit is `u64`, even in 32-bit systems.
                // The number of digits is the number of bytes divided by 8.
                const FROM_DIGITS: usize = $from::BYTES as usize / 8;
                const TO_DIGITS: usize = $to::BYTES as usize / 8;

                let mut digits = [0; TO_DIGITS];
                digits[..FROM_DIGITS].copy_from_slice(value.digits());
                Self::from_digits(digits)
            }
        }
...
mod tests {
    ...
    #[test]
    fn converting_big_uint() {
        // Casting a small number of U256 to U512, should work.
        let u256 = U256::from(42u64);
        let u512: U512 = u256.into();
        assert_eq!(u512, U512::from(42u64));

        // Casting a small number from U512 to U256, should work.
        let u256 = U256::try_from(u512).unwrap();
        assert_eq!(u256, U256::from(42u64));

        // Casting a big number (over the range of U256) from U512 to U256,
        // should fail.
        assert!(U256::try_from(U512::MAX).is_err());
    }
}
